...
static uint32_t battery_level_char_add(ble_bas_t * p_bas, const ble_bas_init_t * p_bas_init)
{
    ...
    //adds the base id to the softdevice stack, puts a reference to where it is
    //into acc_uuid_type
    ...
    attr_md_2.read_perm  = p_bas_init->battery_level_char_attr_md.read_perm;
    attr_md_2.write_perm = p_bas_init->battery_level_char_attr_md.write_perm;
    ...
    memset(&attr_char_value_2, 0, sizeof(attr_char_value));
    ...
    attr_char_value_2.p_value   = &initial_battery_level;
...
uint32_t ble_bas_init(ble_bas_t * p_bas, const ble_bas_init_t * p_bas_init)
{
    uint32_t   err_code;
    ble_uuid_t ble_uuid;
    ble_uuid128_t acc_base_uuid = {{0xAA, 0xCA, 0x55, 0xAC, 0xEE, 0xEF, 0xFE, 0xDE, 0xEE, 0xAF, 0xBE, 0xA0, 0x00, 0x00, 0xA0, 0xEF}}; //nb bytes 12 and 13 are ignored
    ...
    // Add service
    // Add uuid to p_bas struct and add to sd stack
    err_code = sd_ble_uuid_vs_add(&acc_base_uuid, &p_bas->uuid_type);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
